...
class NWLazyMix(Operator, NWBase):
    bl_description = T("通过交互式绘制节点之间的连线来添加混合 RGB/着色器节点")
...
class NWLazyConnect(Operator, NWBase):
    bl_description = T("连接两个节点，无需点击特定插槽（自动确定）")
...
class NWDeleteUnused(Operator, NWBase):
    bl_description = T("删除所有输出未被使用的节点")
    ...
    delete_muted: BoolProperty(
        name=T("删除静音节点"),
        description=T("删除（但重新连接，像 Ctrl-X）所有静音节点"),
        default=True)
    delete_frames: BoolProperty(
        name=T("删除空帧"),
        description=T("删除所有内部没有节点的帧"),
        default=True)
...
class NWSwapLinks(Operator, NWBase):
    bl_description = T("交换两个选中节点的输出连接，或单个节点的两个相似输入")
...
class NWReloadImages(Operator):
    ...
    bl_label = T("重新加载图像")
    bl_description = T("更新所有图像节点以匹配其磁盘上的文件")
...
class NWMergeNodes(Operator, NWBase):
    ...
    bl_label = T("合并节点")
    bl_description = T("合并选中的节点")
    bl_options = {'REGISTER', 'UNDO'}

    mode: EnumProperty(
        name=T("模式"),
        translation_context=i18n_contexts.id_nodetree,
        description=T("所有可能的混合类型、布尔运算和数学运算"),
    ...
    merge_type: EnumProperty(
        name=T("合并类型"),
        description=T("要使用的合并类型"),
        items=(
            ('AUTO', T('自动'), T('自动检测输出类型')),
            ('SHADER', T('着色器'), T('使用相加或混合着色器合并')),
            ('GEOMETRY', T('几何'), T('使用网格布尔或合并几何节点合并')),
            ('MIX', T('混合节点'), T('使用混合节点合并')),
            ('MATH', T('数学节点'), T('使用数学节点合并')),
            ('ZCOMBINE', T('Z-Combine 节点'), T('使用 Z-Combine 节点合并')),
            ('ALPHAOVER', T('Alpha Over 节点'), T('使用 Alpha Over 节点合并')),
        ),
    )
...
class NWBatchChangeNodes(Operator, NWBase):
    ...
    bl_label = T("批量修改")
    bl_description = T("批量修改混合类型和数学运算")
    bl_options = {'REGISTER', 'UNDO'}
...
class NWChangeMixFactor(Operator, NWBase):
    ...
    bl_label = T("修改系数")
    bl_description = T("修改混合节点和混合着色器节点的系数")
    bl_options = {'REGISTER', 'UNDO'}
...
class NWCopySettings(Operator, NWBase):
    ...
    bl_label = T("复制设置")
    bl_description = T("从活动节点复制设置到选中节点")
    bl_options = {'REGISTER', 'UNDO'}
...
class NWCopyLabel(Operator, NWBase):
    ...
    bl_label = T("复制标签")
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = T("将活动节点的标签复制到选中节点")

    option: EnumProperty(
        name=T("选项"),
        description=T("标签名称来源"),
        items=(
            ('FROM_ACTIVE', T('从活动项'), T('从活动节点')),
            ('FROM_NODE', T('从节点'), T('从连接到选中节点的节点')),
            ('FROM_SOCKET', T('从端口'), T('从连接到选中节点的端口')),
        )
    )
...
class NWAddTextureSetup(Operator, NWBase):
    ...
    bl_label = T("纹理设置")
    bl_description = T("为选中着色器添加纹理节点设置")
    bl_options = {'REGISTER', 'UNDO'}

    add_mapping: BoolProperty(
        name=T("添加映射节点"),
        description=T("为纹理创建坐标和映射节点（选中纹理节点时忽略）"),
        default=True)
...
class NWAddPrincipledSetup(Operator, NWBase, ImportHelper):
    ...
    bl_label = T("原理化纹理设置")
    bl_description = T("为原理化 BSDF 添加纹理节点设置")
    bl_options = {'REGISTER', 'UNDO'}
    ...
    relative_path: BoolProperty(
        name=T('相对路径'),
        description=T('尽可能设置相对于 blend 文件的文件路径'),
        default=True
    )
...
class NWAddReroutes(Operator, NWBase):
    ...
    option: EnumProperty(
        ...
        items=[
            ('ALL', T('所有'), T('添加到所有输出')),
            ('LOOSE', T('松散'), T('仅添加到松散输出')),
            ('LINKED', T('链接'), T('仅添加到已链接输出')),
        ]
    )
...
class NWLinkActiveToSelected(Operator, NWBase):
    bl_description = T("根据各种条件将活动节点链接到选中节点")
    bl_idname = "node.nw_link_active_to_selected"
    bl_label = T("链接活动节点到选中项")
    bl_options = {'REGISTER', 'UNDO'}
...
class NWAlignNodes(Operator, NWBase):
    bl_description = T("将选定的节点整齐地排列成行/列")
    bl_idname = "node.nw_align_nodes"
    bl_label = T("对齐节点")
    bl_options = {'REGISTER', 'UNDO'}
    margin: IntProperty(name=T('边距'), default=50, description=T('节点之间的间距'))
...
class NWSelectParentChildren(Operator, NWBase):
    ...
    option: EnumProperty(
        name=T("选项"),
        items=(
            ('PARENT', T('选择父级'), T('选择父级帧')),
            ('CHILD', T('选择子级'), T('选择选中帧的成员')),
        )
    )
...
class NWDetachOutputs(Operator, NWBase):
    bl_description = T("分离选中节点的输出，保持输入连接")
...
class NWLinkToOutputNode(Operator):
    bl_description = T("连接到合成节点或材质输出节点")
...
class NWMakeLink(Operator, NWBase):
    bl_description = T("从一个接口创建链接到另一个接口")
...
class NWAddSequence(Operator, NWBase, ImportHelper):
    ...
    relative_path: BoolProperty(
        name=T('相对路径'),
        description=T('尽可能将文件路径设置为相对于 blend 文件的路径'),
        default=True
    )
...
class NWSaveViewer(bpy.types.Operator, ExportHelper):
    bl_description = T("将当前查看器节点保存为图像文件")
    bl_idname = "node.nw_save_viewer"
    bl_label = T("保存查看器图像")
    filepath: StringProperty(subtype="FILE_PATH")
    filename_ext: EnumProperty(
        name=T("格式"),
        description=T("选择要保存的文件格式"),
        items=(('.bmp', "BMP", ""),
               ('.rgb', 'IRIS', ""),
               ('.png', 'PNG', ""),
               ('.jpg', 'JPEG', ""),
