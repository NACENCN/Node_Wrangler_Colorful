...
def nw_check(cls, context):
    ...
    if space.type != 'NODE_EDITOR':
        cls.poll_message_set(T("当前编辑器不是节点编辑器。"))
        return False
    if space.node_tree is None:
        cls.poll_message_set(T("当前节点编辑器中未找到节点树。"))
        return False
    if space.node_tree.library is not None:
        cls.poll_message_set(T("当前节点树链接自另一个 .blend 文件。"))
        return False
    return True
...
def nw_check_not_empty(cls, context):
    if not context.space_data.edit_tree.nodes:
        cls.poll_message_set(T("当前节点树不包含任何节点。"))
        return False
    return True
...
def nw_check_selected(cls, context, min=1, max=inf):
    ...
    if num_selected < min:
        if min > 1:
            poll_message = tip_(T("必须至少选择 {:d} 个节点。")).format(min)
        else:
            poll_message = tip_(T("必须至少选择一个节点。"))
        cls.poll_message_set(poll_message)
        return False
    if num_selected > max:
        poll_message = tip_(T("已选择 {:d} 个节点，但此操作只能作用于 {:d} 个节点。")).format(
            num_selected, max)
        cls.poll_message_set(poll_message)
        return False
    return True
...
def nw_check_space_type(cls, context, types):
    if context.space_data.tree_type not in types:
        ...
        poll_message = tip_(T("当前节点树类型不支持。\n"
                              "应为 {:s} 之一。")).format(tree_types_str)
        cls.poll_message_set(poll_message)
        return False
    return True
...
def nw_check_node_type(cls, context, type, invert=False):
    if invert and context.active_node.type == type:
        poll_message = tip_(T("活动节点类型不应为 {:s}。")).format(type)
        cls.poll_message_set(poll_message)
        return False
    elif not invert and context.active_node.type != type:
        poll_message = tip_(T("活动节点类型应为 {:s}。")).format(type)
        cls.poll_message_set(poll_message)
        return False
    return True
...
def nw_check_visible_outputs(cls, context):
    if not any(is_visible_socket(out) for out in context.active_node.outputs):
        cls.poll_message_set(T("当前节点没有可见输出。"))
        return False
    return True
...
def nw_check_viewer_node(cls):
    if get_viewer_image() is None:
        cls.poll_message_set(T("未找到查看器图像。"))
        return False
    return True
